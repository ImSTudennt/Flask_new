...
from traitlets import (Bool, Bytes, Dict, HasTraits, Integer, List, Set, Tuple,
                       Unicode)
...
class TestApplication(TestCase):
    ...
    def test_mro_discovery(self):
        ...
        self.assertSequenceEqual(
            class_to_names(app._classes_with_config_traits([Application])),
            ["Application"],
        )
        self.assertSequenceEqual(
        ...
        self.assertSequenceEqual(
            class_to_names(app._classes_with_config_traits([Foo])), ["Foo"]
        )
        self.assertSequenceEqual(
    ...
    def test_config_propagation(self):
        app = MyApp()
        app.parse_command_line(
            ["--i=10", "--Foo.j=10", "--enable=False", "--log-level=50"]
        )
        app.init_foo()
        app.init_bar()
...
def test_show_config_cli():
    out, err, ec = get_output_error_code(
        [sys.executable, "-m", __name__, "--show-config"]
    )
    assert ec == 0
...
@pytest.mark.skipif(
    sys.implementation.name == "pypy", reason="Test does not work on pypy"
)
