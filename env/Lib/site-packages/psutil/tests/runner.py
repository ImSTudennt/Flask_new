...
import psutil
from psutil._common import hilite, print_color, term_supports_colors
from psutil._compat import super
from psutil.tests import (CI_TESTING, import_module_by_path, print_sysinfo,
                          reap_children, safe_rmpath)
...
class ParallelRunner(ColouredTextRunner):
    ...
    def run(self, suite):
        ...
        # run parallel
        cprint(
            "starting parallel tests using %s workers" % NWORKERS, "green", bold=True
        )
        t = time.time()
...
def main():
    ...
    parser.add_option(
        "--parallel", action="store_true", default=False, help="run tests in parallel"
    )
