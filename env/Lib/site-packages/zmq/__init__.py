...
def _delvewheel_init_patch_0_0_22():
    ...
    if sys.version_info[:2] >= (3, 8):
        conda_workaround = os.path.exists(
            os.path.join(sys.base_prefix, "conda-meta")
        ) and (
            sys.version_info[:3] < (3, 8, 13)
            or (3, 9, 0) <= sys.version_info[:3] < (3, 9, 9)
        )
        if conda_workaround:
        ...
        with open(os.path.join(libs_dir, ".load-order-pyzmq-25.1.0")) as file:
